...
import { ChevronRight, History, Trash2, Trophy, Zap } from "lucide-react";
    ...
    <CricketBroadcastCard className={cn(historyCardShellClass, "overflow-hidden p-0")}>
      <div className="flex h-full w-full flex-col">
        <button
          type="button"
          onClick={onOpen}
          className="flex flex-1 flex-col items-center justify-between gap-6 p-5 text-center touch-manipulation transition hover:brightness-105 active:scale-[0.99] sm:gap-6 sm:p-6"
        >
        ...
        <div className="border-t border-[oklch(0.32_0.04_255)] px-4 py-2">
          <button
            type="button"
            onClick={onDelete}
            className="btn-12 btn-12--destructive btn-12--sm inline-flex w-full sm:w-auto"
          >
            <span className="btn-12__label inline-flex items-center gap-1.5 normal-case">
...
export default function MatchHistoryPage({ onBack }: MatchHistoryPageProps) {
  ...
  const pendingDeleteMatch = useMemo(
    () => quickMatches?.find((match) => match.id === pendingDeleteId) ?? null,
    [pendingDeleteId, quickMatches]
  );

  const handleDeleteQuickMatch = () => {
    if (!pendingDeleteId) return;
    setDeleting(true);
    void authenticatedFetch(`/api/matches/quick/${encodeURIComponent(pendingDeleteId)}`, {
      method: "DELETE",
    })
      .then(async (res) => {
        if (!res.ok) {
          const body = (await res.json().catch(() => ({}))) as { error?: string };
          throw new Error(body.error ?? "Could not delete match");
        }
        setQuickMatches(
          (current) => current?.filter((match) => match.id !== pendingDeleteId) ?? []
        );
        appToast.success("Quick match deleted");
        setPendingDeleteId(null);
      })
      .catch((err) =>
        appToast.error(err instanceof Error ? err.message : "Could not delete match")
      )
  ...
  return (
    <CricketPage wide>
                ...
                {quickMatches.map((match) => (
                  <li key={match.id} className="flex min-h-0">
                    <QuickMatchCard
                      ...
                      onDelete={() => setPendingDeleteId(match.id)}
                    />
                  </li>
                ))}
              </ul>
      ...
      <ConfirmDeleteDialog
        ...
        description={
          pendingDeleteMatch
            ? `Remove "${pendingDeleteMatch.label}" from history? This cannot be undone.`
            : "Remove this match from history?"
        }
        confirmLabel={deleting ? "Deleting…" : "Delete"}
        onConfirm={handleDeleteQuickMatch}
        loading={deleting}
      />
    </CricketPage>
  );
