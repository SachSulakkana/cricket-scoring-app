...
/**
 * Effective dismissal for scorecard status. Retired-hurt is cleared to not out
 * once the batter returns (at the crease or has faced again after RH).
 */
export function getEffectiveDismissalBall(
  innings: InningsData,
  playerName: string,
  playerId?: string
): BallData | null {
  const dismissalBalls = innings.balls.filter(
    (ball) => ball.dismissal !== "none" && ball.dismissedPlayer === playerName
  );
  if (dismissalBalls.length === 0) return null;

  const last = dismissalBalls[dismissalBalls.length - 1]!;
  if (last.dismissal !== "retired-hurt") return last;

  const lastIndex = innings.balls.lastIndexOf(last);
  const facedAfterReturn = innings.balls
    .slice(lastIndex + 1)
    .some((ball) => ball.batsmanName === playerName);
  const atCrease =
    playerId != null &&
    (innings.strikerPlayerId === playerId ||
      innings.nonStrikerPlayerId === playerId);

  if (facedAfterReturn || atCrease) return null;
  return last;
}

export function formatBattingDismissalStatus(
  innings: InningsData,
  playerName: string,
  playerId?: string
): string {
  ...
  if (!dismissalBall) return "not out";

  if (dismissalBall.dismissal === "bowled")
    return `Bowled by ${dismissalBall.bowlerName}`;
  if (dismissalBall.dismissal === "lbw")
    return `LBW by ${dismissalBall.bowlerName}`;
  if (dismissalBall.dismissal === "caught")
    return `Caught by ${dismissalBall.fielderName || "Unknown"}, bowled by ${dismissalBall.bowlerName}`;
  if (dismissalBall.dismissal === "stumped")
    return `Stumped by ${dismissalBall.fielderName || "Unknown"}, bowled by ${dismissalBall.bowlerName}`;
  if (dismissalBall.dismissal === "retired-hurt") return "Retired hurt";
  return `Run out by ${dismissalBall.fielderName || "Unknown"}`;
}
